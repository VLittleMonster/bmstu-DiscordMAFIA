...
async def on_message(message):
    ...
    if message.content.startswith("/mafia"):
        ...
        global main_channel, mafia_channel # Перехват глобального канала и канала для мафии
        ...
        global overwrites # Стандартная защита всех каналов
    ...
    if message.content == "/join " + uniqueNumber:
        ...
        # Создание канала для игры и уведомление участников о конце регистрации
            ...
            '"play_room"')
            main_channel = await message.guild.create_text_channel("play_room", overwrites=overwrites)
            mafia_channel = await message.guild.create_text_channel("mafia", overwrites=overwrites)
...
async def on_guild_channel_create(channel):
    ...
    # Удаление 1 - ой скрытой комнаты
    if channel.name == "privet_mir_1":
        await channel.delete()
       
    # Удаление 2 - ой скрытой комнаты
    if channel.name == "privet_mir_2":
        await channel.delete()
       
    elif channel.name == "play_room":
       
        await channel.send(file=discord.File('intro.jpg'))
        ...
        for i in range(len(rolesList)):
            if rolesList[i] == "mafia":
                await mafia_night(channel, playersList[i])
 
        # Знакомство мафии в своем текстовом канале
        await mafiaChannelGreeting(channel, playersList)
   
        # Создание 1 - ой скрытой комнаты
        await sms.guild.create_text_channel("privet_mir_1", overwrites = overwrites)
...
async def on_guild_channel_delete(channel):
    ...
    if channel.name == "privet_mir_1":
        ...
        await main_channel.send(file=discord.File('night.png'))
       
        for i in range(len(rolesList)):
            if rolesList[i] != "" and rolesList[i] != "friendly" and rolesList[i] != "mafia":
                await playersList[i].send("Наступила ночь, пришло время сделать выбор!" + numberedList(playersList))
 
        # Наступление ночи в игре (открытие чата для мафии)
        for i in range(len(rolesList)):
            if rolesList[i] == "mafia":
                await mafia_night(mafia_channel, playersList[i])
                await mafia_channel.send(("Наступила ночь, пришло время сделать выбор!" + numberedList(playersList)))
        ...
        await sms.guild.create_text_channel("privet_mir_2", overwrites = overwrites)
    ...
    # Наступление утра
        ...
        await main_channel.send(file=discord.File('day.png'))
        ...
        for role in rolesList:
            ...
            elif role != "": anotherPlayersCounter += 1
        print(anotherPlayersCounter)
        if mafiaCounter == 0:
